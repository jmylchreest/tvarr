...
//
// Rules use SET_IF_EMPTY keyword syntax so they only fire when programme_category
// is currently empty, preserving real category data from XMLTV sources.
// Note: the ?= shorthand is NOT used here because it misparses when the regex
// condition contains | alternations — use SET_IF_EMPTY ... = instead.
			...
			rules := []models.DataMappingRule{
				{
					...
					Expression:  `channel_group_title matches "(?i)\\b(sport|football|soccer|basket|hockey|racing|tennis|golf|boxing|ufc|mma|nba|nfl|mlb|nhl|mls|f1|formula|dazn|bein|ppv|espn|sporter)" SET_IF_EMPTY programme_category = "Sports"`,
					Priority:    10,
					StopOnMatch: false,
					IsEnabled:   new(true),
					IsSystem:    true,
				},
				{
					...
					Description: "Sets programme_category to 'Movies' (if empty) when the channel's group_title " +
						...
						"cinéma, box office.",
					SourceType:  models.DataMappingRuleSourceTypeEPG,
					Expression:  `channel_group_title matches "(?i)\\b(cinema|movie|film|kino|cine|cin[eé]ma|box.?office)" SET_IF_EMPTY programme_category = "Movies"`,
					Priority:    11,
					StopOnMatch: false,
					IsEnabled:   new(true),
					IsSystem:    true,
				},
				{
					...
					SourceType:  models.DataMappingRuleSourceTypeEPG,
					Expression:  `channel_group_title matches "(?i)\\b(news|haber|noticias|information|actualit)" SET_IF_EMPTY programme_category = "News"`,
					Priority:    12,
					StopOnMatch: false,
					IsEnabled:   new(true),
					IsSystem:    true,
				},
				{
					...
					SourceType:  models.DataMappingRuleSourceTypeEPG,
					Expression:  `channel_group_title matches "(?i)\\b(kids?|cartoon|junior|family|children|disney|bambini|enfants|djecij|cocuk|femij|kinderen|barn)" SET_IF_EMPTY programme_category = "Kids"`,
					Priority:    13,
					StopOnMatch: false,
					IsEnabled:   new(true),
					IsSystem:    true,
				},
				{
					...
					Description: "Sets programme_category to 'Documentary' (if empty) when the channel's group_title " +
						...
						"ντοκιμαντ, belgesel.",
					SourceType:  models.DataMappingRuleSourceTypeEPG,
					Expression:  `channel_group_title matches "(?i)\\b(document|ντοκιμαντ|belgesel)" SET_IF_EMPTY programme_category = "Documentary"`,
					Priority:    14,
					StopOnMatch: false,
					IsEnabled:   new(true),
					IsSystem:    true,
				},
				{
					...
					SourceType:  models.DataMappingRuleSourceTypeEPG,
					Expression:  `channel_group_title matches "(?i)\\b(music|musica|musique|m[uü]zik|μουσικ)" SET_IF_EMPTY programme_category = "Music"`,
					Priority:    15,
					StopOnMatch: false,
					IsEnabled:   new(true),
					IsSystem:    true,
