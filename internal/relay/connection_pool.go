...
// HostLimiter is a simpler interface for just limiting concurrent operations per host.
...
// NewHostLimiter creates a new host limiter.
func NewHostLimiter(maxPerHost int) *HostLimiter {
	return &HostLimiter{
		maxPerHost: maxPerHost,
		hosts:      make(map[string]*hostSemaphore),
	}
}

// Acquire acquires a slot for the given host.
...
// ActiveForHost returns the number of active connections for a host.
func (l *HostLimiter) ActiveForHost(host string) int {
	l.mu.Lock()
	hs, ok := l.hosts[host]
	l.mu.Unlock()
