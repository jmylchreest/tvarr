...
func (m *jobMockJobRepo) HasPendingIngestionJobs(ctx context.Context, sourceIDs []models.ULID) (bool, error) {
	if m.err != nil {
		return false, m.err
	}
	for _, j := range m.jobs {
...
func (m *jobMockStreamSourceRepo) GetByURL(ctx context.Context, url string) (*models.StreamSource, error) {
	for _, s := range m.sources {
		if s.URL == url {
			return s, nil
		}
...
func (m *jobMockProxyRepo) CountByEncodingProfileID(ctx context.Context, profileID models.ULID) (int64, error) {
	return 0, nil
}

func (m *jobMockProxyRepo) GetByEncodingProfileID(ctx context.Context, profileID models.ULID) ([]*models.StreamProxy, error) {
	return nil, nil
}
