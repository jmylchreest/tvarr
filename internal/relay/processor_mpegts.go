...
func (p *MPEGTSProcessor) broadcastToExistingClients(data []byte) {
	...
	for _, client := range clients {
		...
		default:
			...
			const congestionGracePeriod = 15 * time.Second
			congestionDuration := now.Sub(client.congestionStart)
			shouldDisconnect := congestionDuration >= congestionGracePeriod
...
func (p *MPEGTSProcessor) broadcastToClients(data []byte, hasKeyframe bool) {
	...
	for _, client := range clients {
		...
		default:
			...
			// Check if congestion has exceeded grace period (15 seconds)
